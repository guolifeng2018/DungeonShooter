...
{
	public static void SetObjLayer( Transform tran, int layer, bool recursively)
	{
		if ( tran == null )
		{
			return;
    ...
    public static T GetChildComponentAddIfNotExist<T>(GameObject go, string name) where T : Component
    {
        GameObject gameObj = GetChild(go, name);
        return GetComponentAddIfNotExist<T>(gameObj);
    }
    ...
    public static void RevertTransform(Transform trans)
    {
        if (trans == null)
        {
            return;
